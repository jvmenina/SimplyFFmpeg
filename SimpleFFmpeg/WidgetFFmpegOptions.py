from CommonWidgets import Preset, QLabelledLineEdit, QRadioText, QRadioTextButton, States
...
class Widget_FFmpegOptions(QWidget):
    def __init__(self, app_state: States, *args, **kwargs) -> None:
        ...
        presets_overwrite_layout = QHBoxLayout(None)
        main_layout.addLayout(presets_overwrite_layout)
        
        ##### Presets        
        presets_widget = QGroupBox("Presets", self)
        presets_layout = QHBoxLayout(presets_widget)
        presets_overwrite_layout.addWidget(presets_widget, 1)

        self.preset = QComboBox(presets_widget)
        presets_layout.addWidget(self.preset)
        for idx, preset in enumerate(self.states.getPresetsList()):
        ...
        presets_overwrite_layout.addWidget(overwrite_widget)
        ...
        def on_preset_toggle() -> None:
            preset: Preset = self.preset.currentData()
            self.states.preset = preset
            
            if (self.preset.currentIndex() == 0):
                self.video_options_widget.setEnabled(True)
                self.audio_options_widget.setEnabled(True)
            else:
                self.video_options_widget.setEnabled(False)
                self.audio_options_widget.setEnabled(False)
        ...
        on_radiotext_toggle()
        self.video_bitrate_form.buttonToggled.connect(on_radiotext_toggle)
